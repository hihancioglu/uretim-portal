...
def parse_factory_decimal(value, *, label="Değer", decimal_places=5):
    ...
    if not _DECIMAL.fullmatch(text) or ("," in text and "." in text):
        raise forms.ValidationError(f"{label} geçersiz.")
    fraction = text.lstrip("+-").replace(",", ".").partition(".")[2]
    if len(fraction) > decimal_places:
        raise forms.ValidationError(
            f"{label} en fazla {decimal_places} ondalık basamak içerebilir."
        )
    try:
        result = Decimal(text.replace(",", "."))
    except InvalidOperation as exc:
        raise forms.ValidationError(f"{label} geçersiz.") from exc
    if not result.is_finite() or result.adjusted() > 8:
        raise forms.ValidationError(f"{label} numeric(14,5) aralığında olmalıdır.")
    return result
